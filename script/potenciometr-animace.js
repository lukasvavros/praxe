(() => {
  ...
  const SOLUTION_PASSWORD_HASH = '91b927ba802d1dbb2760732182646c1dc9dd63b6345107d6a8eb197afe73ab08';
...
${blink ? `    Serial.print(" V | Prodleva: ");
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      q('#pot-error').textContent = 'Heslo nelze ověřit. Otevři stránku přes HTTPS v aktuálním prohlížeči.';
      return;
    }
    if (input.value !== enteredPassword || form.hidden) return;
    if (!validPassword) { q('#pot-error').textContent = 'Nesprávné heslo. Zkus to znovu.'; input.setAttribute('aria-invalid','true'); input.focus(); return; }
