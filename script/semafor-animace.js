(() => {
  ...
  if (!root) return;
  const SOLUTION_PASSWORD_HASH = '2aa1940b92be49fc24fb07cfb1fa8463332501ba9c3b8cd468941a2fbb8518ab';
  async function verifySolutionPassword(value) {
    if (!globalThis.crypto || !globalThis.crypto.subtle) {
      throw new Error('Ověření hesla vyžaduje zabezpečené připojení HTTPS.');
    }
    const bytes = new TextEncoder().encode(value);
    const digest = await globalThis.crypto.subtle.digest('SHA-256', bytes);
    const hash = Array.from(new Uint8Array(digest), b => b.toString(16).padStart(2, '0')).join('');
    return hash === SOLUTION_PASSWORD_HASH;
  }
  ...
  const rememberKey = 'pra-traffic-solution-unlocked';
  const rememberBox = root.querySelector('[data-remember-solution]');
  ...
  if (hasRememberedUnlock()) {
    rememberBox.checked = true; q('[data-lock-panel]').hidden = true; solution.hidden = false;
  }
  q('[data-lock]').textContent = 'Skrýt a zapomenout odemknutí';
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      validPassword = await verifySolutionPassword(enteredPassword);
    } catch {
      q('#traffic-error').textContent = 'Heslo nelze ověřit. Otevři stránku přes HTTPS v aktuálním prohlížeči.';
      return;
    }
    if (input.value !== enteredPassword || form.hidden) return;
    if (!validPassword) { q('#traffic-error').textContent = 'Nesprávné heslo. Zkus to znovu.'; input.setAttribute('aria-invalid','true'); input.focus(); return; }
  ...
  q('[data-lock]').addEventListener('click', () => { rememberUnlock(false); rememberBox.checked = false; lock(); q('[data-reveal]').focus(); });
