(() => {
  ...
  document.querySelectorAll('[data-serial-demo]').forEach(root => {
    ...
    const tips = {
      1:'Každá společná změna stavu obou LED vytvoří jeden nový řádek v monitoru.',
      ...
      6:'Průběh ukazuje počítadlo nad monitorem. Tento program nic přes Serial nevypisuje. Po skončení obě LED zůstanou zhasnuté.'
    };
    function led(on) {
      all('[data-ls-led]').forEach(el => paintLed(el, on && ['2', '3'].includes(el.getAttribute('data-ls-led'))));
    ...
    function advance() {
      ...
      if (mode === 1) {
        ...
        q('[data-sr-value]').textContent = String(step+1); description = on ? 'Obě LED v poli svítí.' : 'Obě LED v poli nesvítí.';
      } else if (mode === 2) {
        ...
        if (step === 0) { duration = 2000; q('[data-sr-value]').textContent = '0 / 25'; description = 'Čekání 2 sekundy. Obě LED jsou zhasnuté.'; }
        else if (step === 1) { duration = 1000; led(true); description = 'Obě LED svítí 1 sekundu.'; }
        else if (step < 52) {
        ...
        } else { done = true; stop(); elapsed = duration; description = 'Hotovo: 25 bliknutí. Obě LED zůstávají zhasnuté.'; }
      }
      draw();
    }
    function tick() {
