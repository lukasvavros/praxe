(() => {
  ...
  document.querySelectorAll('[data-servo-ex]').forEach(root=>{
    ...
    function leds(green=false,red=false){if(pot)return;q('[data-ex-green]').classList.toggle('on',green);q('[data-ex-red]').classList.toggle('on',red);}
    function draw(){q('[data-ex-arm]').setAttribute('transform',`rotate(${90-current} 210 178)`);q('[data-ex-servo]').setAttribute('aria-label',`Servo v modelu: ${Math.round(current)} stupňů; požadováno ${target}.`);q('[data-ex-target]').textContent=target+'°';}
    function buttons(){if(!pot){q('[data-ex-plus]').disabled=current!==target;q('[data-ex-minus]').disabled=current!==target;}}
    ...
    function tick(now){frame=null;if(previous===null)previous=now;const step=Math.max(0,now-previous)*.25;previous=now;const distance=target-current;if(Math.abs(distance)<=step){finish();return;}current+=Math.sign(distance)*step;draw();frame=requestAnimationFrame(tick);}
    function move(value){target=Math.max(0,Math.min(180,Math.trunc(value)));if(frame!==null)cancelAnimationFrame(frame);frame=null;previous=null;draw();if(current===target||reduced.matches){finish();return;}leds(target>current,target<current);buttons();q('[data-ex-status]').textContent='Pohyb do polohy '+target+'°.';frame=requestAnimationFrame(tick);}
  ...
  examples.forEach(root=>{
    ...
    q('[data-example-form]').addEventListener('submit',e=>{e.preventDefault();if(q('[data-example-password]').value!=='servo519'){q('[data-example-error]').textContent='Nesprávné heslo.';return;}memory(q('[data-example-remember]').checked);display(true);q('[data-hide-example]').focus();});
