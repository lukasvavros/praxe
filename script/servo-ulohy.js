(() => {
  ...
  document.querySelectorAll('[data-servo-ex]').forEach(root=>{
    ...
    function leds(green=false,red=false){if(pot)return;q('[data-ex-green]').classList.toggle('on',green);q('[data-ex-red]').classList.toggle('on',red);root.querySelectorAll('[data-board-led]').forEach(el=>el.classList.toggle('on',el.dataset.boardLed==='green'?green:red));}
